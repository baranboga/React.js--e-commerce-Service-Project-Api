...
public class AuthController : ControllerBase
{
    ...
    {
        if (userName == "baran" && password == "1234")
        {
            try
            {
                ...
                return new AuthModel { Token = token, IsAuthenticated = true, Usta=true };
            }
        ...
        else if (userName == "ahmet" && password == "1234")
        {
            try
            {
                var signingKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_signingKey));
                ...
                var token = new JwtSecurityTokenHandler().WriteToken(jwtSecurityToken);
                return new AuthModel { Token = token, IsAuthenticated = true, Usta=false };
            }
            catch (System.Exception)
            {
